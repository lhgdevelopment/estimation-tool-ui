...
export default function ServiceDeliverableTasksListComponent(props: TServiceDeliverableTasksComponent) {
  const { setEditDataId, listData, setListData, setEditData, editDataId } = props
  const [currentPage, setCurrentPage] = useState(1)
  const [totalPages, setTotalPages] = useState(1)

  const getList = (page = 1) => {
    apiRequest.get(`/service-deliverable-tasks?page=${page}`).then(res => {
  ...
  const onDelete = (i: string) => {
    Swal.fire({
      ...
      if (res.isConfirmed) {
        apiRequest.delete(`/service-deliverable-tasks/${i}`).then(res => {
          Swal.fire({
              ...
              {listData?.map((data: any, index: number) => {
                return (
                  <Box component={'tr'} key={index} className='text-gray-700 dark:text-gray-400'>
                    ...
                    <td className='px-4 py-3 text-sm'>
                      {data?.service_deliverable?.service_scope?.service_group?.service?.name}
                    </td>
                    <td className='px-4 py-3 text-sm'>
                      {data?.service_deliverable?.service_scope?.service_group?.name}
                    </td>
                    <td className='px-4 py-3 text-sm'>
                      <Box sx={{ width: '200px', whiteSpace: 'normal' }}>
                        {data?.service_deliverable?.service_scope?.name.substring(0, 100).length <
                        data?.service_deliverable?.service_scope?.name.length
                          ? data?.service_deliverable?.service_scope?.name.substring(0, 100) + '...'
                          : data?.service_deliverable?.service_scope?.name.substring(0, 100)}
                      </Box>
                    </td>
                    <td className='px-4 py-3 text-sm'>
                      <Box sx={{ width: '200px', whiteSpace: 'normal' }}>
                        {data?.service_deliverable?.name.substring(0, 100).length <
                        data?.service_deliverable?.name.length
                          ? data?.service_deliverable?.name.substring(0, 100) + '...'
                          : data?.service_deliverable?.name.substring(0, 100)}
                      </Box>
                    </td>
