...
export default function ServiceDeliverablesListComponent(props: TServiceDeliverablesComponent) {
  const { setEditDataId, listData, setListData, setEditData, editDataId } = props
  const [currentPage, setCurrentPage] = useState(1)
  const [totalPages, setTotalPages] = useState(1)

  const getList = (page = 1) => {
    apiRequest.get(`/service-deliverables?page=${page}`).then(res => {
  ...
  const onDelete = (i: string) => {
    Swal.fire({
      ...
      if (res.isConfirmed) {
        apiRequest.delete(`/service-deliverables/${i}`).then(res => {
          Swal.fire({
                  ...
                  <Box component={'tr'} key={index} className='text-gray-700 dark:text-gray-400'>
                    ...
                    <td className='px-4 py-3 text-sm'>
                      <Box sx={{ width: '200px', whiteSpace: 'normal' }}>
                        {data?.service_scope?.name.substring(0, 100).length < data?.service_scope?.name.length
                          ? data?.service_scope?.name.substring(0, 100) + '...'
                          : data?.service_scope?.name.substring(0, 100)}
                      </Box>
                    </td>
