...
export default function ProjectListComponent(props: TProjectComponent) {
  const { setEditDataId, listData, setListData, setEditData, editDataId } = props
  const [currentPage, setCurrentPage] = useState<number>(1)
  const [totalPages, setTotalPages] = useState<number>(1)

  const getList = (page = 1) => {
    apiRequest.get(`/projects?page=${page}`).then(res => {
  ...
  const onEdit = (id: string) => {
    ...
    const editData = listData.length ? listData?.filter((data: any) => data['category_id'] == id)[0] : {}
  ...
  const onDelete = (id: string) => {
    Swal.fire({
      ...
      if (res.isConfirmed) {
        apiRequest.delete(`/projects/${id}`).then(res => {
          Swal.fire({
